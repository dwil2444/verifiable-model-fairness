...
def load_datasets(device, opts):
  ...
  try:
    ...
    raw_data = data_class.get_raw_data(opts.jigsaw_dir, test=opts.test)
...
class JigsawDataset(TextClassificationDataset):
  ...
  """
  @classmethod
  def read_text(cls, jigsaw_dir, split):
    if split == 'test':
        fname = 'test.csv'
    elif split == 'train':
        fname = 'train.csv'
    ...
    df = pd.read_csv(f'{jigsaw_dir}/{fname}')
    data = []
    num_words = 0
    for i in range(len(df)):
        label = df.iloc[i]['toxicity']
        x_raw = df.iloc[i]['text']
        x_toks = word_tokenize(x_raw)
        num_words += len(x_toks)
        data.append((' '.join(x_toks), label))
